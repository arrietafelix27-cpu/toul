...
export default function EditComboPage() {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            // Combo + items + all products/variants in parallel
            const [comboRes, itemsRes, prodsRes, varsRes] = await Promise.all([
                supabase.from('combos').select('*').eq('id', comboId).single(),
                supabase.from('combo_items').select('*').eq('combo_id', comboId),
            ...
            setName(combo.name)
            setComboPrice(String(Math.round(combo.sale_price)))
            setOriginalImageUrl(combo.image_url || null)
            if (combo.image_url) setImagePreview(combo.image_url)

            // Hydrate category from combo (migration_v8 added category_id to combos).
            // Falls back to first category if combo has none and there are categories.
            ...
            // Hydrate selectedItems from combo_items
            const lookupByKey = new Map(items.map(i => [i.key, i]))
            const existing: ComboItem[] = (itemsRes.data || []).map(ci => {
                const key = ci.product_id ? `p-${ci.product_id}` : `v-${ci.variant_id}`
                const found = lookupByKey.get(key)
                if (found) {
                    return { ...found, quantity: ci.quantity }
                }
                // Item points to product/variant that no longer exists or is inactive — keep with placeholder
                return {
                    key,
                    type: ci.product_id ? 'product' : 'variant',
    ...
    async function handleSave() {
        ...
        // UPDATE combo
        const { error: comboErr } = await supabase.from('combos').update({
            name: name.trim(),
        ...
        setTimeout(() => router.push(`/products/combo/${comboId}`), 1800)
    }
    ...
    if (dataLoading) return (
        <div style={{ position: 'relative', minHeight: '100dvh' }}>
            ...
            <div style={{ padding: '20px 18px', maxWidth: 520, margin: '0 auto' }}>
                <div className="skeleton" style={{ height: 36, width: 180, borderRadius: 12, marginBottom: 28 }} />
                <div className="skeleton" style={{ height: 86, width: 86, borderRadius: 18, marginBottom: 20 }} />
        ...
        <div style={{ position: 'relative', minHeight: '100dvh' }}>
            ...
            <div style={{
                ...
                <motion.div
                    ...
                    <Link href={`/products/combo/${comboId}`}
